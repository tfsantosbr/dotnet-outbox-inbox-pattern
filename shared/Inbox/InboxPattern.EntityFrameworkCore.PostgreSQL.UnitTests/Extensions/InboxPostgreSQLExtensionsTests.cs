...
using InboxPattern.Abstractions;
...
namespace InboxPattern.EntityFrameworkCore.PostgreSQL.UnitTests.Extensions;
...
public class InboxPostgreSQLExtensionsTests
{
    ...
    public void UsePostgreSQLStorage_WithOptions_AppliesOptions()
    {
        ...
        // Verifica que o registro foi criado (smoke test — options são encapsuladas no factory)
        var descriptor = services.FirstOrDefault(d => d.ServiceType == typeof(IInboxStorage));
        descriptor.Should().NotBeNull();
    }
